...
{"mcId":"MC1234567","title":"[Platform] Titel [MC1234567]","platform":"platform","roadmapId":"id of null","roadmapUrl":"https://www.microsoft.com/microsoft-365/roadmap","plannerTask":"[Platform] Titel [MC1234567]","planning":["Targeted Release: ...","Algemeen beschikbaar: ..."],"oneLiner":"Max 2 zinnen die beschrijven waar dit item over gaat, geschikt als opmerking in een Planner-taak. Zakelijk, concreet, geen jargon.","omschrijvingIntro":"tekst","omschrijvingBullets":["punt1","punt2"],"omschrijvingSlot":"tekst of lege string","impactOrganisaties":"laag/gemiddeld/hoog - toelichting","impactTechnisch":"tekst","impactFunctioneel":"tekst","impactBeheer":["actie1","actie2"],"relevantieSCore":3,"relevantieUitleg":"Korte uitleg waarom dit item relevant of minder relevant is voor een enterprise Microsoft 365 omgeving. Max 1 zin.","links":[{"label":"Microsoft Learn - naam","url":"https://..."},{"label":"Microsoft Message Center - MC1234567","url":null}],"geenSpecifiekeLearnPagina":false}

De relevantieScore is een getal van 1 tot 5:
1 = Nauwelijks relevant (bijv. Dynamics 365, Power Platform specifiek)
2 = Beperkt relevant
...
def fetch_mc_list(count):
    resp = requests.get("https://mc.merill.net", timeout=10)
...
def fetch_item_images(url):
    """Haal afbeelding-URLs op van een MC item pagina."""
    try:
...
def send_teams_notification(webhook_url, new_items):
    """Stuur een Teams notificatie via webhook."""
    ...
    payload = {
        ...
        "themeColor": "0078D4",
        "summary": f"{len(new_items)} nieuwe MC analyses gereed",
        "sections": [{
            "activityTitle": f"MC Analyzer: {len(new_items)} nieuwe analyses",
    ...
    try:
        ...
        print(f"Teams notificatie mislukt: {e}")
...
def run_analysis(api_key, items, force, webhook_url=""):
    ...
    for item in items:
        ...
        try:
            ...
            time.sleep(0.5)
            # Gebruik geanalyseerde titel als bestandsnaam
...
def get_items():
    ...
    try:
        ...
        # Markeer alle opgehaalde items als gezien
...
def download_zip():
    ...
    if not ids:
        return "Geen items opgegeven", 400
    buf = io.BytesIO()
    state = load_state()
...
def settings():
    ...
    return jsonify({"api_key": api_key, "count": "50", "webhook_url": webhook})
